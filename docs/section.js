...
async function loadSectionPage(){
  ...
  try{
    ...
    grid.innerHTML=(data.classes||[]).map(item=>{
      ...
      return `<a class="class-door" href="/alalam-alsaghir/class/?section=${activeSection}&class=${item.id}" style="--door-color:${escapeHtml(item.color)}"><span class="door-number">${item.id}</span><span class="door-copy"><strong>${escapeHtml(item.name)}</strong><small>${assignments} ${escapeHtml(sectionMeta(activeSection).item)} • ${submissions} مشاركة</small></span><span class="door-arrow">←</span></a>`;
    }).join("");
