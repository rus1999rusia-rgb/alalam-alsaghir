...
function renderClassAssignments(){
  ...
  $("#assignmentsGrid").innerHTML=items.length?items.map(item=>`<article class="assignment-card section-${classSection}"><span class="card-label">${item.class_id?escapeHtml(getClassName(classData,item.class_id)):"كل الفصول"}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)||"التفاصيل تحددها المعلمة."}</p>${item.due_date?`<div class="due">آخر موعد: ${escapeHtml(formatDate(item.due_date))}</div>`:""}<a class="text-link" href="/alalam-alsaghir/submit/?section=${classSection}&class=${classId}&assignment=${encodeURIComponent(item.id)}">إرسال المشاركة ←</a></article>`).join(""):emptyState(meta.emptyTitle,"ستضيف المعلمة المحتوى الجديد هنا.",meta.icon);
}
