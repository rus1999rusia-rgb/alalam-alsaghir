...
function renderClassPage(){
  ...
  $("#submitLink").href=`/submit?section=${classSection}&class=${classId}`;
  $("#switchClasses").innerHTML=(classData.classes||[]).map(item=>`<a class="mini-class ${Number(item.id)===classId?"active":""}" href="/class?section=${classSection}&class=${item.id}" style="--class-color:${escapeHtml(item.color)}">${escapeHtml(item.name)}</a>`).join("");
...
function renderClassAssignments(){
  ...
  $("#assignmentsGrid").innerHTML=items.length?items.map(item=>{const urls=Array.isArray(item.image_urls)?item.image_urls.filter(Boolean):[];const gallery=urls.length?`<div class="assignment-gallery ${urls.length===1?"single":""}">${urls.map((url,index)=>`<a href="${escapeHtml(url)}" target="_blank" rel="noopener"><img src="${escapeHtml(url)}" alt="${escapeHtml(item.title)} — صورة ${index+1}" loading="lazy"></a>`).join("")}</div>`:"";return`<article class="assignment-card section-${classSection}">${gallery}<div class="assignment-card-body"><span class="card-label">${item.class_id?escapeHtml(getClassName(classData,item.class_id)):"كل الفصول"}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)||"التفاصيل تحددها المعلمة."}</p>${item.due_date?`<div class="due">آخر موعد: ${escapeHtml(formatDate(item.due_date))}</div>`:""}<a class="text-link" href="/submit?section=${classSection}&class=${classId}&assignment=${encodeURIComponent(item.id)}">إرسال المشاركة ←</a></div></article>`}).join(""):emptyState(meta.emptyTitle,"ستضيف المعلمة المحتوى الجديد هنا.",meta.icon);
}
